...
// ShopScreen is now rendered directly by CollectionGrid, so App doesn't import it

export default function App() {
  const [showButtons, setShowButtons] = useState(false)
  const [view, setView] = useState<'home' | 'collection'>('home') // Only home or collection at top level
  const [collectionCategory, setCollectionCategory] = useState<'organic' | 'inorganic' | null>(null) // For CollectionGrid
  ...
  const handleCloseCollection = () => {
    ...
    setView('home')
    setHoveredColor(null) // Clear cursor color
  }
  ...
  return (
    ...
    >
      ...
      <AnimatePresence mode="wait">
        {view === 'home' && (
          <motion.div
            key="home-view" // Unique key for AnimatePresence
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        {view === 'collection' && collectionCategory && (
          <CollectionGrid
            category={collectionCategory}
            onClose={handleCloseCollection} // This onClose will set App's view back to 'home'
            onHoverColor={setHoveredColor}
          />
        )}

        {/* ShopScreen is rendered internally by CollectionGrid based on its own state */}
      </AnimatePresence>
    </main>
  )
}
